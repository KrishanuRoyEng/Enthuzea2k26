...
export default function ShowcaseVideo() {
  ...
  // Use IntersectionObserver instead of Framer Motion scroll spring
  ...
  useEffect(() => {
    ...
    const handleResume = () => {
      ...
      if (videoRef.current && isVisible) videoRef.current.play().catch(() => {});
    };
  ...
  return (
    ...
    >
      {/* ─── SMOOTH SECTION TRANSITION (Gradient blend upward) ─── */}
      ...
      {/* Left Light System */}
      <div className="absolute top-2 sm:top-10 left-2 sm:left-20 z-30 w-20 h-20 sm:w-28 sm:h-28">
        <StageLightBase className="absolute inset-0 w-full h-full z-10" />
        
        <div 
          className="absolute inset-0 w-full h-full origin-center will-change-transform transform-gpu animate-stage-swing-left"
        >
      ...
      {/* Right Light System */}
      <div className="absolute top-2 sm:top-10 right-2 sm:right-20 z-30 w-20 h-20 sm:w-28 sm:h-28">
        <StageLightBase className="absolute inset-0 w-full h-full z-10 scale-x-[-1]" />
        
        <div 
          className="absolute inset-0 w-full h-full origin-center will-change-transform transform-gpu animate-stage-swing-right"
        >
      ...
      {/* Giant ambient glow behind the player (optimized radial gradient instead of blur) */}
      ...
      <div 
        className={`relative z-20 w-[calc(100%-2rem)] sm:w-[calc(100%-4rem)] max-w-5xl flex flex-col items-center transition-all duration-700 ease-out will-change-[opacity,transform] ${
          isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
        }`}
      >
        <h2 className="text-2xl sm:text-4xl font-display text-gradient-gold font-bold mb-6 sm:mb-8 uppercase tracking-[0.3em] text-center drop-shadow-lg">
          The Grand Experience
        </h2>
        
        {/* Video Container (The "Screen" above the stage) */}
        <div className="relative w-full aspect-video rounded-xl sm:rounded-2xl overflow-hidden shadow-[0_0_100px_rgba(245,166,35,0.25)] ring-4 ring-white/10 z-20 bg-black group">
          ...
          {/* Cinematic Scanlines */}
          <div className="absolute inset-0 pointer-events-none z-20 opacity-[0.15] bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(255,0,0,0.06),rgba(0,255,0,0.02),rgba(0,0,255,0.06))] bg-[length:100%_4px,3px_100%]" />
          <video
            ref={videoRef}
          ...
          <div className="absolute inset-0 pointer-events-none ring-inset ring-1 ring-white/20 rounded-xl sm:rounded-2xl z-20 shadow-[inset_0_0_50px_rgba(0,0,0,0.8)]" />
          <div className="absolute inset-0 pointer-events-none bg-gradient-to-t from-marigold/10 to-transparent mix-blend-overlay z-20" />
        </div>

        {/* ─── THE 3D STAGE BENEATH THE VIDEO ─── */}
        <div className="relative w-[110%] max-w-6xl h-16 sm:h-24 mt-[-20px] sm:mt-[-40px] z-10">
          {/* Glowing floor reflection (optimized without expensive CSS blur) */}
